...
[![Image Site](https://img.shields.io/badge/Image-%E7%8B%AC%E7%AB%8B%E9%83%A8%E7%BD%B2-10b981?style=for-the-badge)](https://image.luoyv.net/)

</div>

<br>

> 说明：这是面向落羽小站实际部署的定制备份仓库，不是上游原版的镜像仓库。上游项目与 MIT 许可信息见 [UPSTREAM.md](UPSTREAM.md)。
...
### 🎨 绘图与编辑体验
- **文本生图与参考图**：支持文本生成、参考图上传、剪贴板粘贴、拖拽上传和多图参考。
- **遮罩编辑**：内置可视化遮罩编辑器，并对尺寸、透明度和模型限制做预处理。
- **批量与迭代**：支持一次生成多张图片，将满意结果快速转为下一轮参考图。
- **流式预览**：在支持的接口模式下接收中间步骤图像，减少长请求等待的不确定感。

### 🤖 Agent 多轮绘图
- **多轮上下文**：支持对话式绘图，能引用前面轮次的图片或通过 `@` 选择画廊图片。
- **对话/绘图分离配置**：支持对话模型和绘图模型使用不同供应商，例如对话使用 Claude，绘图使用 GPT 图像模型。
- **Agent 快捷开关**：Agent 菜单栏加入绘图开关和搜索快捷开关，可快速控制是否允许调用绘图工具或 Web 搜索。
- **并发生成工具**：Agent 可在一次任务中并发生成多张关联图像，并继续处理依赖步骤。
- **分支与重生成**：编辑或重生成历史消息时创建可切换分支，避免不同分支图片互相串用。
- **画廊联动**：Agent 生成内容会进入画廊；删除对话和删除画廊任务时尽量保持引用关系干净。
...
构建产物输出到 `dist/`，可交给 Nginx、Docker 或其它静态服务器部署。`dist/` 不进入 Git 仓库。
...
同步前应先确认本仓库的 Sub2 嵌入、账号同步、代理配置和本地缓存逻辑不会被上游覆盖。更多维护说明见 [UPSTREAM.md](UPSTREAM.md)。
...
```text
https://image.luoyv.net/?apiUrl={address}&apiKey={key}&model={model}
```

在 Sub2 内嵌场景中，通常由站内账号同步和同源代理负责注入配置，不需要用户手动拼接 API Key。

```text
https://sub2.luoyv.net/gallery
```
...
本仓库基于上游最新版，移植了 luoyv66/sub2-image 的 Sub2 定制层：
