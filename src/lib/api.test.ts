...
describe('callImageApi', () => {
  ...
  it('ignores non-JSON SSE relay chunks after a valid Images API stream result', async () => {
    const streamBody = [
      'data: {"object":"image.generation.result","created":1779551140,"model":"gpt-image-2","data":[{"b64_json":"ZmluYWw="}],"size":"1024x1536","quality":"medium","output_format":"png"}',
      '',
      'event: ping',
      'data: relay keepalive',
      '',
      'data: [DONE]',
      '',
    ].join('\n')
  ...
  it('uses Gemini native v1beta image generation through a root API proxy', async () => {
    vi.stubEnv('VITE_API_PROXY_AVAILABLE', 'true')
    vi.stubEnv('VITE_API_PROXY_LOCKED', 'true')
    vi.stubEnv('VITE_API_PROXY_TARGET', 'https://sub2.luoyv.net')
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({
      candidates: [{
        content: {
          parts: [{ inlineData: { data: 'aW1hZ2U=' } }],
        },
      }],
    }), {
      status: 200,
  ...
  it('parses Gemini generatedImages image bytes responses', async () => {
    vi.stubEnv('VITE_API_PROXY_AVAILABLE', 'true')
    vi.stubEnv('VITE_API_PROXY_LOCKED', 'true')
    vi.stubEnv('VITE_API_PROXY_TARGET', 'https://sub2.luoyv.net')
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(new Response(JSON.stringify({
  ...
  it('parses Gemini proxy responses that return OpenAI-style image URLs', async () => {
    vi.stubEnv('VITE_API_PROXY_AVAILABLE', 'true')
    vi.stubEnv('VITE_API_PROXY_LOCKED', 'true')
    vi.stubEnv('VITE_API_PROXY_TARGET', 'https://sub2.luoyv.net')
    const fetchMock = vi.spyOn(globalThis, 'fetch')
    ...
    expect(fetchMock.mock.calls[1][0]).toBe('https://images.example.com/cat.png')
    expect(result.images[0]).toMatch(/^data:image\/png;base64,/)
    expect(result.rawImageUrls).toEqual(['https://images.example.com/cat.png'])
  })
