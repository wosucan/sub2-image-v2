...
describe('buildApiUrl', () => {
  ...
  it('adds v1 when proxying through a root API target', () => {
    expect(buildApiUrl('http://api.example.com', 'images/generations', null, true)).toBe(
      '/api-proxy/v1/images/generations',
    )
  })

  it('does not add v1 when the proxy target already includes an API version', () => {
    expect(
      buildApiUrl(
        '',
        'responses',
    ...
    ).toBe('/api-proxy/responses')
  })

  it('keeps explicit Gemini v1beta paths under a root proxy target', () => {
    expect(
      buildApiUrl(
        '',
    ...
    ).toBe('/api-proxy/v1beta/models/gemini-3-pro-image-preview:generateContent')
  })
  ...
  it('does not add v1 before explicit versioned API paths when proxy is disabled', () => {
    expect(buildApiUrl('http://api.example.com/v1', 'v1beta/models', null, false)).toBe(
      'http://api.example.com/v1beta/models',
    )
  })
