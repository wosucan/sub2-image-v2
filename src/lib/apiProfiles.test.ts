...
describe('getActiveApiProfile', () => {
  it('does not let top-level split models override the selected profile', () => {
    ...
    expect(active.id).toBe(profileB.id)
    expect(active.imageModel).toBeUndefined()
    expect(active.chatModel).toBeUndefined()
  })

  it('preserves top-level split models for legacy settings without profiles', () => {
    const settings = normalizeSettings({
    ...
    expect(settings.profiles[0].imageModel).toBe('legacy-image')
    expect(settings.profiles[0].chatModel).toBe('legacy-chat')
    expect(active.imageModel).toBe('legacy-image')
    expect(active.chatModel).toBe('legacy-chat')
  })
})
