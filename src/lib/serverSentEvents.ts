export interface ReadJsonServerSentEventsOptions {
  ...
  /** 部分中转网关会在有效 SSE 数据后附加纯文本 keepalive/状态块，开启后忽略这些块。 */
  ignoreNonJsonChunks?: boolean
}
...
export async function readJsonServerSentEvents(
  ...
  const processBlock = async (block: string) => {
    ...
    try {
      ...
      // 仅忽略明确的 keepalive/ping 噪声块；真正的畸形 JSON 数据仍需原样抛出。
      if (options.ignoreNonJsonChunks && /\b(keepalive|ping)\b/i.test(block)) return
