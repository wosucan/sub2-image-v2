...
export const DEFAULT_GROK_IMAGE_MODEL = 'grok-2-image'
export const DEFAULT_GROK_CHAT_MODEL = 'grok-4'
export const DEFAULT_GEMINI_IMAGE_MODEL = 'gemini-2.0-flash-exp-image-generation'
export const DEFAULT_GEMINI_CHAT_MODEL = 'gemini-2.5-pro'
export const DEFAULT_CLAUDE_MODEL = 'claude-sonnet-4-5'
export const DEFAULT_OPENAI_PROFILE_ID = 'default-openai'
export const DEFAULT_API_TIMEOUT = 900

const BUILT_IN_PROVIDER_IDS = new Set<ApiProvider>(['openai', 'sb2api-async', 'fal', 'grok', 'gemini', 'claude'])

const SPLIT_MODEL_PROVIDER_IDS = new Set<ApiProvider>(['openai', 'grok', 'gemini', 'claude'])
const AGENT_IMAGE_PROVIDER_IDS = new Set<ApiProvider>(['openai', 'grok', 'gemini', 'fal'])
function cleanSplitProviderModelValue(provider: ApiProvider, value: unknown): string | undefined {
...
function cleanProviderDrafts(drafts: ApiProfile['providerDrafts']): ApiProfile['providerDrafts'] {
  if (!drafts) return undefined
  const entries = Object.entries(drafts).map(([provider, draft]) => {
    const typedProvider = provider as ApiProvider
    if (!draft) return [typedProvider, draft] as const
    if (!SPLIT_MODEL_PROVIDER_IDS.has(typedProvider)) return [typedProvider, draft] as const
    return [typedProvider, {
      ...draft,
      model: undefined,
      imageModel: cleanSplitProviderModelValue(typedProvider, draft.imageModel),
      chatModel: cleanSplitProviderModelValue(typedProvider, draft.chatModel),
    }] as const
  })
  return Object.fromEntries(entries) as ApiProfile['providerDrafts']
}
const CUSTOM_PROVIDERS_ENABLED = true
...
export function normalizeCustomProviderDefinitions(input: unknown): CustomProviderDefinition[] {
  if (!CUSTOM_PROVIDERS_ENABLED) return []
...
export function getDefaultModelForProvider(provider: ApiProvider, apiMode: ApiMode = 'images'): string {
  if (provider === 'grok') return apiMode === 'responses' ? DEFAULT_GROK_CHAT_MODEL : DEFAULT_GROK_IMAGE_MODEL
  if (provider === 'gemini') return apiMode === 'responses' ? DEFAULT_GEMINI_CHAT_MODEL : DEFAULT_GEMINI_IMAGE_MODEL
...
export function switchApiProfileProvider(profile: ApiProfile, provider: ApiProvider, customProvider?: CustomProviderDefinition): ApiProfile {
  const nextProvider = BUILT_IN_PROVIDER_IDS.has(provider) || customProvider ? provider : 'openai'
  ...
  const savedDraft = providerDrafts[nextProvider]
...
function normalizeProviderDraft(
  ...
  const imageModel = cleanSplitProviderModelValue(provider, input.imageModel)
  const chatModel = cleanSplitProviderModelValue(provider, input.chatModel)
...
function inferBuiltInProviderFromProfile(record: Record<string, unknown>, provider: ApiProvider): ApiProvider {
  if (provider !== 'openai') return provider
  const name = typeof record.name === 'string' ? record.name.toLowerCase() : ''
  const model = typeof record.model === 'string' ? record.model.toLowerCase() : ''
  const marker = `${name} ${model}`
  if (marker.includes('gemini')) return 'gemini'
  if (marker.includes('grok') || marker.includes('xai') || marker.includes('x-ai')) return 'grok'
  if (marker.includes('claude') || marker.includes('anthropic')) return 'claude'
  return provider
}
...
export function normalizeApiProfile(
  ...
  const normalizedProvider: ApiProvider = BUILT_IN_PROVIDER_IDS.has(rawProvider) || customProviderIds.has(rawProvider) ? rawProvider : 'openai'
  const provider: ApiProvider = inferBuiltInProviderFromProfile(record, normalizedProvider)
  ...
  const legacyModel = typeof record.model === 'string' && record.model.trim() ? record.model.trim() : defaults.model
  ...
  return {
    ...
    model: legacyModel,
    imageModel: cleanSplitProviderModelValue(provider, record.imageModel),
    chatModel: cleanSplitProviderModelValue(provider, record.chatModel),
    ...
    providerDrafts: cleanProviderDrafts(normalizeProviderDrafts(record.providerDrafts, customProviderIds, nativeTransparentProviderIds)),
  }
}
...
export function normalizeSettings(input: Partial<AppSettings> | unknown): AppSettings {
  ...
  const legacyProfile = createDefaultOpenAIProfile({
    ...
    imageModel: cleanSplitProviderModelValue('openai', record.imageModel),
    chatModel: cleanSplitProviderModelValue('openai', record.chatModel),
  ...
  return {
    ...
    agentTextProfileId,
    agentImageGeneration: typeof record.agentImageGeneration === 'boolean' ? record.agentImageGeneration : true,
    agentImageProfileId,
    profiles,
    activeProfileId,
  }
...
export function isOpenAICompatibleProvider(settings: Partial<AppSettings> | unknown, provider: ApiProvider): boolean {
  return provider === 'openai' || provider === 'grok' || provider === 'gemini' || provider === 'claude' || Boolean(getCustomProviderDefinition(settings, provider))
}
...
export function importCustomProviderSettingsFromJson(
  ...
  if (!CUSTOM_PROVIDERS_ENABLED) throw new Error('当前部署已禁用自定义服务商')
  let parsed: unknown
  try {
...
export function getActiveApiProfile(settings: Partial<AppSettings> | unknown): ApiProfile {
  ...
  return {
    ...
    imageModel: cleanSplitProviderModelValue(profile.provider, profile.imageModel),
    chatModel: cleanSplitProviderModelValue(profile.provider, profile.chatModel),
...
export function getProfileImageModel(profile: ApiProfile): string {
  return profile.imageModel?.trim() || profile.model.trim()
}

export function getProfileChatModel(profile: ApiProfile): string {
  return profile.chatModel?.trim() || profile.model.trim()
}

export function isAgentImageApiProfileCandidate(profile: ApiProfile): boolean {
  if (profile.provider === 'claude') return false
  return AGENT_IMAGE_PROVIDER_IDS.has(profile.provider) || !BUILT_IN_PROVIDER_IDS.has(profile.provider)
}
...
export function getAgentImageApiProfile(settings: Partial<AppSettings> | unknown): ApiProfile | null {
  ...
  return normalized.profiles.find((profile) => profile.id === normalized.agentImageProfileId && isAgentImageApiProfileCandidate(profile)) ?? null
}
