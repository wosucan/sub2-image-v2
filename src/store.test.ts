...
describe('gallery image profile validation', () => {
  beforeEach(async () => {
    await clearTasks()
    await clearImages()
    await clearAgentConversations()
    const imageOnlyProfile = createDefaultOpenAIProfile({
      id: 'image-only-profile',
      name: 'Image only',
      apiKey: 'test-key',
    ...
    useStore.setState({
      settings: normalizeSettings({
        ...DEFAULT_SETTINGS,
        profiles: [imageOnlyProfile],
        activeProfileId: imageOnlyProfile.id,
      }),
      prompt: '画一只兔子',
      inputImages: [],
      maskDraft: null,
  ...
  it('submits gallery image tasks without requiring a chat model', async () => {
    await submitTask()
