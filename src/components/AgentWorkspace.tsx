...
import { TrashIcon, DownloadIcon, EditIcon, ChevronDownIcon, ChevronLeftIcon, ChevronRightIcon, SidebarLeftIcon, FavoriteIcon, CloseIcon, CopyIcon, RefreshIcon, ArrowDownIcon, HistoryIcon } from './icons'
...
function getAgentRoundDisplayName(round: AgentRound | null | undefined) {
  return round?.chatModel?.trim() || round?.chatApiProfileName?.trim() || 'Agent'
}
...
export default function AgentWorkspace() {
  ...
  return (
    ...
    >
      ...
      {/* Left Sidebar */}
      <aside className={`agent-sidebar-panel fixed bottom-0 left-0 z-50 flex w-4/5 max-w-[320px] flex-col overflow-hidden rounded-tr-2xl border-r border-gray-200 bg-white/95 shadow-2xl backdrop-blur transition-transform duration-300 dark:border-white/[0.08] dark:bg-gray-950/95 ${!sidebarCollapsed ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="pl-[max(1rem,env(safe-area-inset-left))] flex h-full min-h-0 w-full flex-col">
          <div className="shrink-0">
            <div className="flex h-14 items-center justify-between gap-2 px-4">
              ...
              <button type="button" onClick={createConversation} className="-mr-2 rounded-lg px-2.5 py-1.5 text-sm font-medium text-gray-600 transition-colors hover:bg-gray-100 hover:text-gray-900 dark:text-gray-300 dark:hover:bg-white/[0.04] dark:hover:text-gray-100" title="新建对话">
                新建对话
              </button>
            </div>
          </div>
          ...
          {filteredConversations.map((item) => {
            ...
            return (
              ...
              >
                ...
                <div className={`flex shrink-0 items-center gap-1 overflow-hidden transition-all duration-150 ${agentEditingConversationId === item.id ? 'w-6 opacity-100' : 'w-[4.5rem] opacity-100'}`}>
                  {agentEditingConversationId === item.id ? (
                    <AgentActionButton
                      tooltip="确认"
                      onClick={(e) => e.stopPropagation()}
      ...
      <section className="min-w-0 flex-1 flex flex-col relative">
        {/* Conversation Header Toggles */}
        <div className={`agent-conversation-header fixed left-0 right-0 z-30 transition-all duration-300 ease-in-out ${showHistoryModal && mobileTopBarVisible ? 'overflow-visible' : 'overflow-hidden'} ${mobileTopBarVisible ? 'max-h-16 opacity-100' : 'max-h-0 opacity-0 pointer-events-none'}`}>
          <div
          ...
          >
            ...
            <div className="relative flex shrink-0 items-center gap-1">
              <button type="button" onClick={createConversation} className="rounded-lg px-2.5 py-1.5 text-sm font-medium text-gray-600 transition-colors hover:bg-gray-100 hover:text-gray-900 dark:text-gray-300 dark:hover:bg-white/[0.04] dark:hover:text-gray-100" title="新建对话">
                新建对话
              </button>
              <button
        ...
        <div 
          ...
          className="flex-1 space-y-4 overflow-visible pt-16 pb-[calc(var(--input-bar-clearance,12rem)+1.5rem)] px-1 lg:px-4"
          ...
          {!conversation ? (
            ...
            (() => {
              ...
              const renderedMessages = activeMessages.map((message) => {
                ...
                const messageLabel = isAssistant ? getAgentRoundDisplayName(round) : sub2ApiUser?.username || sub2ApiUser?.email || '用户'
                const messageTimeLabel = isAssistant ? '回复' : '发送'
                return (
                  <div key={message.id} className={`flex w-full mb-6 ${isAssistant ? 'justify-start' : 'justify-end'}`}>
                    ...
                    <div className="mt-2 text-right text-[11px] leading-none text-gray-400 dark:text-gray-500">
                      {messageTimeLabel} {formatMessageTime(message.createdAt)} · 第 {round?.index ?? '?'} 轮
                    </div>
                    ...
                    <div key={`running-${round.id}`} className="flex w-full justify-start mb-6">
                      <article className="flex min-w-[16rem] max-w-[95%] flex-col rounded-2xl rounded-tl-sm border border-gray-200 bg-white/70 p-4 dark:border-white/[0.08] dark:bg-white/[0.03] md:max-w-[85%] lg:max-w-[75%]">
                        <div className="mb-2 text-sm text-gray-500 dark:text-gray-400">
                          <span className="text-blue-600 dark:text-blue-400 font-semibold">{getAgentRoundDisplayName(round)}</span> <span className="ml-1 font-normal opacity-60">· 第 {round.index} 轮</span>
                        </div>
