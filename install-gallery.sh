...
echo "==> [2/5] 写入 docker-compose.override.yml (域名: $DOMAIN, 不改动原配置)"
...
echo "==> [3/5] 准备 docker-compose.yml (上游未提供, 由示例生成, 不覆盖已有)"
cd "$APP_DIR"
if [ ! -f docker-compose.yml ]; then
  cp docker-compose.example.yml docker-compose.yml
  echo "    已从 docker-compose.example.yml 生成"
else
  echo "    已存在, 跳过"
fi

echo "==> [4/5] 构建并启动 (首次需 npm install, 约 3-10 分钟)"
